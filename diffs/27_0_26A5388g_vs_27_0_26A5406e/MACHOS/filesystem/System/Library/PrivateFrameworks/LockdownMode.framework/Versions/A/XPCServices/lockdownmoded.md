## lockdownmoded

> `/System/Library/PrivateFrameworks/LockdownMode.framework/Versions/A/XPCServices/lockdownmoded`

### Sections with Same Size but Changed Content

- `__TEXT.__objc_methlist`
- `__TEXT.__cstring`
- `__TEXT.__swift5_entry`
- `__TEXT.__constg_swiftt`
- `__TEXT.__swift5_fieldmd`
- `__TEXT.__swift5_builtin`
- `__TEXT.__swift5_assocty`
- `__TEXT.__swift5_proto`
- `__TEXT.__swift5_types`
- `__TEXT.__swift5_capture`
- `__TEXT.__swift_as_entry`
- `__TEXT.__swift_as_ret`
- `__TEXT.__swift_as_cont`
- `__TEXT.__swift5_protos`
- `__TEXT.__swift5_mpenum`
- `__DATA_CONST.__const`
- `__DATA_CONST.__objc_classlist`
- `__DATA_CONST.__objc_protolist`
- `__DATA_CONST.__objc_protorefs`
- `__DATA.__objc_const`
- `__DATA.__objc_data`
- `__DATA.__bss`
- `__DATA.__common`

```diff

-128.0.5.0.0
-  __TEXT.__text: 0x3f8c8
-  __TEXT.__auth_stubs: 0x12d0
-  __TEXT.__objc_stubs: 0xd80
+128.0.8.0.0
+  __TEXT.__text: 0x40354
+  __TEXT.__auth_stubs: 0x1360
+  __TEXT.__objc_stubs: 0xe40
   __TEXT.__objc_methlist: 0x3ac
-  __TEXT.__const: 0x1671
+  __TEXT.__const: 0x16a1
   __TEXT.__cstring: 0x1166
-  __TEXT.__swift5_typeref: 0x863
+  __TEXT.__swift5_typeref: 0x87d
   __TEXT.__swift5_entry: 0x8
   __TEXT.__objc_classname: 0x4c9
-  __TEXT.__objc_methname: 0x131d
+  __TEXT.__objc_methname: 0x138d
   __TEXT.__objc_methtype: 0x59f
   __TEXT.__constg_swiftt: 0xc28
   __TEXT.__swift5_reflstr: 0x59b
   __TEXT.__swift5_fieldmd: 0x558
-  __TEXT.__oslogstring: 0x2e4c
+  __TEXT.__oslogstring: 0x2e8c
   __TEXT.__swift5_builtin: 0x78
   __TEXT.__swift5_assocty: 0x108
   __TEXT.__swift5_proto: 0xcc

   __TEXT.__swift5_protos: 0x4
   __TEXT.__swift5_mpenum: 0x10
   __TEXT.__gcc_except_tab: 0x20
-  __TEXT.__unwind_info: 0x940
-  __TEXT.__eh_frame: 0xfd0
+  __TEXT.__unwind_info: 0x958
+  __TEXT.__eh_frame: 0xfe0
   __DATA_CONST.__const: 0x15b0
   __DATA_CONST.__objc_classlist: 0x80
   __DATA_CONST.__objc_protolist: 0xb0
   __DATA_CONST.__objc_imageinfo: 0x8
   __DATA_CONST.__objc_protorefs: 0x60
-  __DATA_CONST.__auth_got: 0x978
-  __DATA_CONST.__got: 0x428
-  __DATA_CONST.__auth_ptr: 0x358
+  __DATA_CONST.__auth_got: 0x9c0
+  __DATA_CONST.__got: 0x450
+  __DATA_CONST.__auth_ptr: 0x370
   __DATA.__objc_const: 0xf98
-  __DATA.__objc_selrefs: 0x4d0
+  __DATA.__objc_selrefs: 0x500
   __DATA.__objc_data: 0x4b0
-  __DATA.__data: 0x1562
+  __DATA.__data: 0x1572
   __DATA.__bss: 0x1920
   __DATA.__common: 0x18
   - /System/Library/Frameworks/Accounts.framework/Versions/A/Accounts

   - /usr/lib/swift/libswift_Concurrency.dylib
   - /usr/lib/swift/libswiftos.dylib
   - /usr/lib/swift/libswiftsimd.dylib
-  Functions: 789
-  Symbols:   556
-  CStrings:  568
+  Functions: 795
+  Symbols:   572
+  CStrings:  575
 
Symbols:
+ _$s10Foundation6LocaleV10identifierACSS_tcfC
+ _$s10Foundation6LocaleV12LanguageCodeV10identifierSSvg
+ _$s10Foundation6LocaleV12LanguageCodeVMa
+ _$s10Foundation6LocaleV12LanguageCodeVMn
+ _$s10Foundation6LocaleV19_bridgeToObjectiveCSo8NSLocaleCyF
+ _$s10Foundation6LocaleV8LanguageV12languageCodeAC0cE0VSgvg
+ _$s10Foundation6LocaleV8LanguageVMa
+ _$s10Foundation6LocaleV8languageAC8LanguageVvg
+ _$s10Foundation6LocaleVMa
+ _$s10Foundation6LocaleVMn
+ _$sSKsSS7ElementRtzrlE6joined9separatorS2S_tF
+ _$sSS10FoundationE6format6locale9argumentsS2Sh_AA6LocaleVSghSays7CVarArg_pGhtcfC
+ _$sSayxGSKsMc
+ _$sSis23CustomStringConvertiblesWP
+ _$ss5Int64VN
+ _$ss5Int64Vs7CVarArgsWP
+ _OBJC_CLASS_$_NSNumber
+ _OBJC_CLASS_$_NSNumberFormatter
- _$sSS10FoundationE25localizedStringWithFormatyS2S_s7CVarArg_pdtFZ
- _$sSis7CVarArgsWP
CStrings:
+ "/AppleInternal/Library/BuildRoots/<BUILDROOT>/Library/Caches/com.apple.xbs/TemporaryDirectory.aqL70o/Sources/AppleCredentialManager_ClientLibs/ACMLib/ACMLib.c"
+ "/AppleInternal/Library/BuildRoots/<BUILDROOT>/Library/Caches/com.apple.xbs/TemporaryDirectory.aqL70o/Sources/AppleCredentialManager_ClientLibs/common/LibCall.c"
+ "EXEMPTION_REVOKED_MIXED_V2"
+ "EXEMPTION_REVOKED_UNNAMED_V2"
+ "EXEMPTION_REVOKED_V2"
+ "developmentLocalization"
+ "initWithInteger:"
+ "preferredLocalizations"
+ "setLocale:"
+ "setNumberStyle:"
+ "stringFromItems:"
+ "stringFromNumber:"
+ "syncWithKVS: Latest state on other devices: %{public}s (%{public}s), but this device's state is more recent (%{public}s). Overriding with `%{public}s`…"
+ "syncWithKVS: skipNextKVSSync unset"
- "/AppleInternal/Library/BuildRoots/<BUILDROOT>/Library/Caches/com.apple.xbs/TemporaryDirectory.j7jzB6/Sources/AppleCredentialManager_ClientLibs/ACMLib/ACMLib.c"
- "/AppleInternal/Library/BuildRoots/<BUILDROOT>/Library/Caches/com.apple.xbs/TemporaryDirectory.j7jzB6/Sources/AppleCredentialManager_ClientLibs/common/LibCall.c"
- "EXEMPTION_REVOKED"
- "EXEMPTION_REVOKED_MIXED"
- "EXEMPTION_REVOKED_UNNAMED"
- "Latest state on other devices: %{public}s (%{public}s), but this device's state is more recent (%{public}s). Overriding with `%{public}s`…"
- "localizedStringByJoiningStrings:"
```
