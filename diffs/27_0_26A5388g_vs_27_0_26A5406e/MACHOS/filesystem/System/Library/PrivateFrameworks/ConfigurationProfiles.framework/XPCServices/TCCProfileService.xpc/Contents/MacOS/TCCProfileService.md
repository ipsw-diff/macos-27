## TCCProfileService

> `/System/Library/PrivateFrameworks/ConfigurationProfiles.framework/XPCServices/TCCProfileService.xpc/Contents/MacOS/TCCProfileService`

### Sections with Same Size but Changed Content

- `__TEXT.__objc_methlist`
- `__TEXT.__unwind_info`
- `__DATA_CONST.__const`
- `__DATA_CONST.__objc_classlist`
- `__DATA_CONST.__objc_superrefs`
- `__DATA_CONST.__objc_arraydata`
- `__DATA_CONST.__objc_dictobj`
- `__DATA_CONST.__objc_arrayobj`
- `__DATA.__objc_const`
- `__DATA.__objc_data`

```diff

-910.0.0.0.0
-  __TEXT.__text: 0x5984
+913.0.0.0.0
+  __TEXT.__text: 0x59dc
   __TEXT.__auth_stubs: 0x2b0
-  __TEXT.__objc_stubs: 0x860
+  __TEXT.__objc_stubs: 0x8a0
   __TEXT.__objc_methlist: 0x178
   __TEXT.__const: 0x50
   __TEXT.__gcc_except_tab: 0x54
-  __TEXT.__cstring: 0x75d
+  __TEXT.__cstring: 0x771
   __TEXT.__oslogstring: 0xfcf
-  __TEXT.__objc_methname: 0x82c
+  __TEXT.__objc_methname: 0x84d
   __TEXT.__objc_classname: 0x12
-  __TEXT.__objc_methtype: 0x151
+  __TEXT.__objc_methtype: 0x143
   __TEXT.__unwind_info: 0x178
   __DATA_CONST.__const: 0x1f0
-  __DATA_CONST.__cfstring: 0x980
+  __DATA_CONST.__cfstring: 0x9a0
   __DATA_CONST.__objc_classlist: 0x8
   __DATA_CONST.__objc_imageinfo: 0x8
   __DATA_CONST.__objc_superrefs: 0x8

   __DATA_CONST.__auth_got: 0x168
   __DATA_CONST.__got: 0x98
   __DATA.__objc_const: 0xf8
-  __DATA.__objc_selrefs: 0x248
+  __DATA.__objc_selrefs: 0x258
   __DATA.__objc_ivar: 0xc
   __DATA.__objc_data: 0x50
   __DATA.__bss: 0x50

   - /usr/lib/libsandbox.1.dylib
   Functions: 89
   Symbols:   77
-  CStrings:  227
+  CStrings:  229
 
Functions:
~ sub_10000359c : 468 -> 524
~ sub_100003dbc -> sub_100003df4 : 2496 -> 2528
CStrings:
+ "PayloadOrganization"
+ "_processInfoDict:payload:organization:"
+ "length"
+ "organization"
- "@32@0:8@16@24"
- "_processInfoDict:payload:"
```
