## Setup Assistant

> `/System/Library/CoreServices/Setup Assistant.app/Contents/MacOS/Setup Assistant`

### Sections with Same Size but Changed Content

- `__TEXT.__objc_methtype`
- `__TEXT.__objc_classname`
- `__TEXT.__gcc_except_tab`
- `__TEXT.__swift5_protos`
- `__DATA_CONST.__objc_classlist`
- `__DATA_CONST.__objc_nlclslist`
- `__DATA_CONST.__objc_catlist`
- `__DATA_CONST.__objc_protolist`
- `__DATA_CONST.__objc_protorefs`
- `__DATA_CONST.__objc_superrefs`
- `__DATA_CONST.__objc_intobj`
- `__DATA_CONST.__objc_arraydata`
- `__DATA_CONST.__objc_arrayobj`
- `__DATA_CONST.__objc_dictobj`

```diff

-7544.0.0.0.0
-  __TEXT.__text: 0x19570c
-  __TEXT.__auth_stubs: 0x2c20
-  __TEXT.__objc_stubs: 0x26600
-  __TEXT.__objc_methlist: 0x1667c
+7547.0.0.0.0
+  __TEXT.__text: 0x195688
+  __TEXT.__auth_stubs: 0x2c40
+  __TEXT.__objc_stubs: 0x26620
+  __TEXT.__objc_methlist: 0x16684
   __TEXT.__objc_methtype: 0x73d2
-  __TEXT.__cstring: 0x35735
+  __TEXT.__cstring: 0x3595a
   __TEXT.__objc_classname: 0x430f
-  __TEXT.__objc_methname: 0x3acc5
+  __TEXT.__objc_methname: 0x3ac25
   __TEXT.__ustring: 0x29e
-  __TEXT.__const: 0x7834
+  __TEXT.__const: 0x78d4
   __TEXT.__gcc_except_tab: 0x14e8
   __TEXT.__dlopen_cstrs: 0x105
   __TEXT.__oslogstring: 0x57
-  __TEXT.__swift5_typeref: 0x2616
-  __TEXT.__constg_swiftt: 0x389c
-  __TEXT.__swift5_reflstr: 0x3f84
-  __TEXT.__swift5_fieldmd: 0x2810
-  __TEXT.__swift5_types: 0x340
-  __TEXT.__swift5_assocty: 0x348
-  __TEXT.__swift5_builtin: 0x230
-  __TEXT.__swift5_proto: 0x310
-  __TEXT.__swift_as_entry: 0x5c4
-  __TEXT.__swift_as_ret: 0x3e0
-  __TEXT.__swift_as_cont: 0x6f8
-  __TEXT.__swift5_capture: 0xadc
+  __TEXT.__swift5_typeref: 0x2640
+  __TEXT.__constg_swiftt: 0x3908
+  __TEXT.__swift5_reflstr: 0x3fa4
+  __TEXT.__swift5_fieldmd: 0x2828
+  __TEXT.__swift5_types: 0x344
+  __TEXT.__swift5_assocty: 0x360
+  __TEXT.__swift5_builtin: 0x244
+  __TEXT.__swift5_proto: 0x31c
+  __TEXT.__swift_as_entry: 0x5bc
+  __TEXT.__swift_as_ret: 0x3d0
+  __TEXT.__swift_as_cont: 0x6e4
+  __TEXT.__swift5_capture: 0xaa0
   __TEXT.__swift5_protos: 0x14
-  __TEXT.__unwind_info: 0x6c90
-  __TEXT.__eh_frame: 0x4710
-  __DATA_CONST.__const: 0xc0b0
-  __DATA_CONST.__cfstring: 0x1fd60
+  __TEXT.__unwind_info: 0x6c80
+  __TEXT.__eh_frame: 0x4628
+  __DATA_CONST.__const: 0xbfe8
+  __DATA_CONST.__cfstring: 0x1fd80
   __DATA_CONST.__objc_classlist: 0xa98
   __DATA_CONST.__objc_nlclslist: 0x8
   __DATA_CONST.__objc_catlist: 0x40

   __DATA_CONST.__objc_dictobj: 0x2f8
   __DATA_CONST.__objc_floatobj: 0x10
   __DATA_CONST.__objc_doubleobj: 0x20
-  __DATA_CONST.__auth_got: 0x1620
-  __DATA_CONST.__got: 0x13e0
-  __DATA_CONST.__auth_ptr: 0x480
-  __DATA.__objc_const: 0x6d9e0
-  __DATA.__objc_selrefs: 0xc730
-  __DATA.__objc_ivar: 0x1718
-  __DATA.__objc_data: 0x7568
-  __DATA.__data: 0x9160
-  __DATA.__bss: 0x4700
-  __DATA.__common: 0x340
+  __DATA_CONST.__auth_got: 0x1630
+  __DATA_CONST.__got: 0x13e8
+  __DATA_CONST.__auth_ptr: 0x490
+  __DATA.__objc_const: 0x6da20
+  __DATA.__objc_selrefs: 0xc728
+  __DATA.__objc_ivar: 0x1714
+  __DATA.__objc_data: 0x75c0
+  __DATA.__data: 0x91a0
+  __DATA.__bss: 0x4878
+  __DATA.__common: 0x328
   - /System/Library/Frameworks/AVFoundation.framework/Versions/A/AVFoundation
   - /System/Library/Frameworks/AVKit.framework/Versions/A/AVKit
   - /System/Library/Frameworks/Accounts.framework/Versions/A/Accounts

   - /usr/lib/swift/libswift_Concurrency.dylib
   - /usr/lib/swift/libswiftos.dylib
   - /usr/lib/swift/libswiftsimd.dylib
-  Functions: 11344
-  Symbols:   1509
-  CStrings:  16365
+  Functions: 11336
+  Symbols:   1513
+  CStrings:  16373
 
Symbols:
+ _$s10Foundation4DateVMa
+ _$s10Foundation4DateVMn
+ _$s10Foundation4DateVSQAAMc
+ _$s12LockdownMode0aB7ManagerC22threatNotificationDate10Foundation0F0VSgvg
CStrings:
+ "$__lazy_storage_$_variant"
+ "-[ToAnotherMacController viewWillAppear]_block_invoke_2"
+ "/AppleInternal/Library/BuildRoots/<BUILDROOT>/Library/Caches/com.apple.xbs/TemporaryDirectory.iN3TKM/Sources/AppleKeyStore_libs/aeskeywrap.c"
+ "/AppleInternal/Library/BuildRoots/<BUILDROOT>/Library/Caches/com.apple.xbs/TemporaryDirectory.iN3TKM/Sources/AppleKeyStore_libs/backup_serialize.c"
+ "/AppleInternal/Library/BuildRoots/<BUILDROOT>/Library/Caches/com.apple.xbs/TemporaryDirectory.iN3TKM/Sources/AppleKeyStore_libs/platform/platform.c"
+ "/AppleInternal/Library/BuildRoots/<BUILDROOT>/Library/Caches/com.apple.xbs/TemporaryDirectory.iN3TKM/Sources/AppleKeyStore_libs/platform/platform_lib.c"
+ "/AppleInternal/Library/BuildRoots/<BUILDROOT>/Library/Caches/com.apple.xbs/TemporaryDirectory.iN3TKM/Sources/AppleKeyStore_libs/shared_crypto.c"
+ "BOOL MBSystemUpdateMatchesReleaseKind(NSString *__strong, NSString *__strong, MBReleaseUpdateKind)"
+ "CONTINUE_MIGRATION_BUTTON"
+ "Forcing lockdown pane to show due to ForceLockdownMode debug override"
+ "GLASS_SELECTION_PREVIEW_ACCESSIBILITY_LABEL"
+ "LOCKDOWN_MODE_ON_DETAIL"
+ "LOCKDOWN_MODE_ON_DETAIL_THREAT"
+ "LOCKDOWN_MODE_ON_TITLE"
+ "MESSAGE_QUIT_MIGRATION_TITLE"
+ "No usable previous OS version (%@); treating as not an update."
+ "Regulatory item safety settings skipped via MDM"
+ "Showing informational lockdown mode pane, this is a major or E update"
+ "Skipping lockdown pane, Lockdown Mode is on for this Mac but this is not a major or E update"
+ "Update %@ -> %@ matches release kind 0x%lx: %@"
+ "checkmark.shield.fill"
+ "hasThreatNotification"
+ "initWithAdmUserManager:launchPreferencesHandler:"
+ "initWithCountryManager:perUserState:safetySettingsPresenter:skipKeyManager:"
+ "initWithCountryManager:perUserState:skipKeyManager:"
+ "isEnterprise:"
+ "lockdownPaneNeedsToShow"
+ "lockdownPaneVariant"
+ "variant"
- "-[ToAnotherMacController viewWillAppear]_block_invoke"
- "/AppleInternal/Library/BuildRoots/<BUILDROOT>/Library/Caches/com.apple.xbs/TemporaryDirectory.tSt0vh/Sources/AppleKeyStore_libs/aeskeywrap.c"
- "/AppleInternal/Library/BuildRoots/<BUILDROOT>/Library/Caches/com.apple.xbs/TemporaryDirectory.tSt0vh/Sources/AppleKeyStore_libs/backup_serialize.c"
- "/AppleInternal/Library/BuildRoots/<BUILDROOT>/Library/Caches/com.apple.xbs/TemporaryDirectory.tSt0vh/Sources/AppleKeyStore_libs/platform/platform.c"
- "/AppleInternal/Library/BuildRoots/<BUILDROOT>/Library/Caches/com.apple.xbs/TemporaryDirectory.tSt0vh/Sources/AppleKeyStore_libs/platform/platform_lib.c"
- "/AppleInternal/Library/BuildRoots/<BUILDROOT>/Library/Caches/com.apple.xbs/TemporaryDirectory.tSt0vh/Sources/AppleKeyStore_libs/shared_crypto.c"
- "Apple Account Charm"
- "Forcing lockdown pane to show due to cookie file"
- "Skipping lockdown pane, lockdown mode already enabled"
- "TB,V_shouldSetCharm"
- "_shouldSetCharm"
- "cancelButtonString"
- "didSeeLockdownMode is false, continuing"
- "initWithCountryManager:perUserState:"
- "initWithCountryManager:perUserState:safetySettingsPresenter:"
- "initWithUseragent:admUserManager:launchPreferencesHandler:"
- "initWithUseragent:admUserManager:underlyingManager:launchPreferencesHandler:"
- "lockdownPaneNeedsToShowWithCompletionHandler:"
- "setDisplayImage:"
- "setShouldSetCharm:"
- "shouldSetCharm"
```
