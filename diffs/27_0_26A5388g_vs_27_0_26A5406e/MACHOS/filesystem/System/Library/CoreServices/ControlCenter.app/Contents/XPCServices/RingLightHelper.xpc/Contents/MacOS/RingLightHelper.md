## RingLightHelper

> `/System/Library/CoreServices/ControlCenter.app/Contents/XPCServices/RingLightHelper.xpc/Contents/MacOS/RingLightHelper`

### Sections with Same Size but Changed Content

- `__TEXT.__objc_methlist`
- `__TEXT.__swift5_entry`
- `__TEXT.__constg_swiftt`
- `__TEXT.__swift5_builtin`
- `__TEXT.__swift5_fieldmd`
- `__TEXT.__swift5_proto`
- `__TEXT.__swift5_types`
- `__TEXT.__swift5_assocty`
- `__TEXT.__swift_as_entry`
- `__TEXT.__swift_as_ret`
- `__TEXT.__swift_as_cont`
- `__TEXT.__swift5_protos`
- `__TEXT.__swift5_mpenum`
- `__DATA_CONST.__objc_classlist`
- `__DATA_CONST.__objc_protolist`
- `__DATA_CONST.__objc_protorefs`
- `__DATA_CONST.__got`
- `__DATA_CONST.__auth_ptr`
- `__DATA.__objc_const`
- `__DATA.__objc_selrefs`
- `__DATA.__objc_data`
- `__DATA.__bss`

```diff

-808.400.0.0.0
-  __TEXT.__text: 0x36e68
-  __TEXT.__auth_stubs: 0x1970
+810.0.8.400.0
+  __TEXT.__text: 0x38338
+  __TEXT.__auth_stubs: 0x19a0
   __TEXT.__objc_stubs: 0xe20
   __TEXT.__objc_methlist: 0x350
   __TEXT.__swift5_entry: 0x8
-  __TEXT.__const: 0x5010
+  __TEXT.__const: 0x5030
   __TEXT.__constg_swiftt: 0x1970
-  __TEXT.__swift5_typeref: 0x1970
+  __TEXT.__swift5_typeref: 0x197c
   __TEXT.__swift5_builtin: 0xf0
   __TEXT.__swift5_reflstr: 0xc9a
   __TEXT.__swift5_fieldmd: 0x12e0

   __TEXT.__swift5_assocty: 0x318
   __TEXT.__objc_methtype: 0x295
   __TEXT.__cstring: 0xa0c
-  __TEXT.__swift5_capture: 0x408
-  __TEXT.__oslogstring: 0x4ea
+  __TEXT.__swift5_capture: 0x478
+  __TEXT.__oslogstring: 0x63a
   __TEXT.__swift_as_entry: 0x18
   __TEXT.__swift_as_ret: 0x14
   __TEXT.__swift_as_cont: 0x18
   __TEXT.__swift5_protos: 0x10
   __TEXT.__swift5_mpenum: 0x4c
-  __TEXT.__unwind_info: 0x13b0
-  __TEXT.__eh_frame: 0xa38
-  __DATA_CONST.__const: 0x3200
+  __TEXT.__unwind_info: 0x13f0
+  __TEXT.__eh_frame: 0xac8
+  __DATA_CONST.__const: 0x3318
   __DATA_CONST.__objc_classlist: 0x98
   __DATA_CONST.__objc_protolist: 0x48
   __DATA_CONST.__objc_imageinfo: 0x8
   __DATA_CONST.__objc_protorefs: 0x28
-  __DATA_CONST.__auth_got: 0xcc0
+  __DATA_CONST.__auth_got: 0xcd8
   __DATA_CONST.__got: 0x460
   __DATA_CONST.__auth_ptr: 0x7d8
   __DATA.__objc_const: 0x1528
   __DATA.__objc_selrefs: 0x4a8
   __DATA.__objc_data: 0xaf8
-  __DATA.__data: 0x1f70
+  __DATA.__data: 0x1fb0
   __DATA.__common: 0x110
   __DATA.__bss: 0x7040
   - /System/Library/Frameworks/AVFoundation.framework/Versions/A/AVFoundation

   - /usr/lib/swift/libswift_DarwinFoundation3.dylib
   - /usr/lib/swift/libswiftos.dylib
   - /usr/lib/swift/libswiftsimd.dylib
-  Functions: 2179
-  Symbols:   211
-  CStrings:  375
+  Functions: 2209
+  Symbols:   213
+  CStrings:  382
 
Symbols:
+ _NSStringFromRect
+ _swift_retain_n
CStrings:
+ "Dismissing window"
+ "Moving window to %{public}u, %{public}s"
+ "No target display set for window and no main screen available"
+ "No target display set for window; defaulting to main display"
+ "Presenting window at %{public}s"
+ "Setting ring light active for %{public}s"
+ "Setting ring light inactive for %{public}s"
```
