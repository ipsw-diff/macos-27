## TCCAuthorizationService

> `/System/Library/PrivateFrameworks/TCCAuthorizationService.framework/Versions/A/TCCAuthorizationService`

### Sections with Same Size but Changed Content

- `__TEXT.__objc_methlist`
- `__TEXT.__const`
- `__TEXT.__constg_swiftt`
- `__TEXT.__swift5_typeref`
- `__TEXT.__swift5_proto`
- `__TEXT.__swift_as_entry`
- `__TEXT.__swift_as_ret`
- `__TEXT.__swift_as_cont`
- `__TEXT.__eh_frame`
- `__DATA_CONST.__objc_classlist`
- `__DATA_CONST.__objc_protolist`
- `__DATA_CONST.__objc_protorefs`
- `__AUTH_CONST.__const`
- `__AUTH_CONST.__objc_const`
- `__AUTH.__objc_data`
- `__AUTH.__data`
- `__DATA.__data`

```diff

-910.0.0.0.0
-  __TEXT.__text: 0x17cec
+913.0.0.0.0
+  __TEXT.__text: 0x1832c
   __TEXT.__objc_methlist: 0x2dc
   __TEXT.__const: 0x11d0
-  __TEXT.__cstring: 0xff7
+  __TEXT.__cstring: 0x1097
   __TEXT.__constg_swiftt: 0x974
   __TEXT.__swift5_typeref: 0x554
   __TEXT.__swift5_builtin: 0x78

   __TEXT.__swift_as_entry: 0x1c
   __TEXT.__swift_as_ret: 0x20
   __TEXT.__swift_as_cont: 0x28
-  __TEXT.__unwind_info: 0x5b0
+  __TEXT.__unwind_info: 0x5b8
   __TEXT.__eh_frame: 0x690
   __TEXT.__objc_stubs: 0x0
   __TEXT.__auth_stubs: 0x0
   __TEXT.__objc_classname: 0x0
   __TEXT.__objc_methname: 0x0
   __TEXT.__objc_methtype: 0x0
-  __DATA_CONST.__const: 0xe0
+  __DATA_CONST.__const: 0xf0
   __DATA_CONST.__objc_classlist: 0x70
   __DATA_CONST.__objc_protolist: 0xb0
   __DATA_CONST.__objc_imageinfo: 0x8
-  __DATA_CONST.__objc_selrefs: 0x308
+  __DATA_CONST.__objc_selrefs: 0x328
   __DATA_CONST.__objc_protorefs: 0x58
   __DATA_CONST.__got: 0x0
   __AUTH_CONST.__const: 0xae8
   __AUTH_CONST.__objc_const: 0x14e8
-  __AUTH_CONST.__auth_got: 0x5e8
+  __AUTH_CONST.__auth_got: 0x618
   __AUTH.__objc_data: 0xf18
   __AUTH.__data: 0x570
   __DATA.__data: 0x770
   __DATA.__bss: 0x14a0
   __DATA.__common: 0x10
+  - /System/Library/Frameworks/AppKit.framework/Versions/C/AppKit
   - /System/Library/Frameworks/CoreLocation.framework/Versions/A/CoreLocation
   - /System/Library/Frameworks/Foundation.framework/Versions/C/Foundation
   - /System/Library/Frameworks/NetworkExtension.framework/Versions/A/NetworkExtension
+  - /System/Library/Frameworks/Security.framework/Versions/A/Security
   - /System/Library/PrivateFrameworks/BaseBoard.framework/Versions/A/BaseBoard
   - /System/Library/PrivateFrameworks/BoardServices.framework/Versions/A/BoardServices
   - /System/Library/PrivateFrameworks/ConfigurationProfiles.framework/Versions/A/ConfigurationProfiles
+  - /System/Library/PrivateFrameworks/DMCUtilities.framework/Versions/A/DMCUtilities
   - /System/Library/PrivateFrameworks/ManagedSettingsObjC.framework/Versions/A/ManagedSettingsObjC
   - /System/Library/PrivateFrameworks/TCC.framework/Versions/A/TCC
   - /usr/lib/libSystem.B.dylib

   - /usr/lib/swift/libswiftAccelerate.dylib
   - /usr/lib/swift/libswiftCore.dylib
   - /usr/lib/swift/libswiftCoreFoundation.dylib
+  - /usr/lib/swift/libswiftCoreImage.dylib
   - /usr/lib/swift/libswiftCoreLocation.dylib
   - /usr/lib/swift/libswiftDispatch.dylib
   - /usr/lib/swift/libswiftIOKit.dylib

   - /usr/lib/swift/libswiftObjectiveC.dylib
   - /usr/lib/swift/libswiftObservation.dylib
   - /usr/lib/swift/libswiftQuartzCore.dylib
+  - /usr/lib/swift/libswiftSpatial.dylib
   - /usr/lib/swift/libswiftUniformTypeIdentifiers.dylib
   - /usr/lib/swift/libswiftXPC.dylib
   - /usr/lib/swift/libswift_Builtin_float.dylib
   - /usr/lib/swift/libswift_Concurrency.dylib
   - /usr/lib/swift/libswiftos.dylib
   - /usr/lib/swift/libswiftsimd.dylib
-  Functions: 600
-  Symbols:   491
-  CStrings:  84
+  Functions: 605
+  Symbols:   504
+  CStrings:  87
 
Symbols:
+ _OBJC_CLASS_$_DMCComposedIdentifier
+ _OBJC_CLASS_$_NSWorkspace
+ _SecCodeCopyDesignatedRequirement
+ _SecRequirementCopyString
+ _SecStaticCodeCreateWithPath
+ __swift_FORCE_LOAD_$_swiftCoreImage
+ __swift_FORCE_LOAD_$_swiftCoreImage_$_TCCAuthorizationService
+ __swift_FORCE_LOAD_$_swiftSpatial
+ __swift_FORCE_LOAD_$_swiftSpatial_$_TCCAuthorizationService
+ _objc_msgSend$URLForApplicationWithBundleIdentifier:
+ _objc_msgSend$bundleID
+ _objc_msgSend$initWithSigningIdentifier:designatedRequirement:
+ _objc_msgSend$newComposedIdentifier:
+ _objc_msgSend$sharedWorkspace
- _objc_msgSend$initWithSigningIdentifier:
CStrings:
+ "#ManagedTCCDefaults Couldn't determine designated requirement for "
+ "; not saving LocalNetwork authorization"
+ "folder.badge.gearshape"
```
