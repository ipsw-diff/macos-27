## assessmentagent

> `/usr/libexec/assessmentagent`

### Sections with Same Size but Changed Content

- `__TEXT.__objc_methlist`
- `__TEXT.__swift5_entry`
- `__TEXT.__swift5_builtin`
- `__DATA_CONST.__cfstring`
- `__DATA_CONST.__objc_protolist`
- `__DATA_CONST.__objc_protorefs`
- `__DATA_CONST.__objc_superrefs`
- `__DATA_CONST.__objc_arraydata`
- `__DATA_CONST.__objc_arrayobj`
- `__DATA.__objc_data`

```diff

-56.0.0.0.0
-  __TEXT.__text: 0xb5058
-  __TEXT.__auth_stubs: 0x2210
-  __TEXT.__objc_stubs: 0x2820
+56.0.3.0.0
+  __TEXT.__text: 0xbd1a8
+  __TEXT.__auth_stubs: 0x2230
+  __TEXT.__objc_stubs: 0x28e0
   __TEXT.__objc_methlist: 0xfc8
-  __TEXT.__const: 0x9178
-  __TEXT.__cstring: 0x3721
-  __TEXT.__objc_methname: 0x3b69
-  __TEXT.__objc_classname: 0x185f
+  __TEXT.__const: 0x9708
+  __TEXT.__cstring: 0x39d1
+  __TEXT.__objc_methname: 0x3c69
+  __TEXT.__objc_classname: 0x18ef
   __TEXT.__objc_methtype: 0xd03
   __TEXT.__gcc_except_tab: 0x20
-  __TEXT.__swift5_typeref: 0x3e98
+  __TEXT.__swift5_typeref: 0x3fe2
   __TEXT.__swift5_entry: 0x8
-  __TEXT.__constg_swiftt: 0x4b48
+  __TEXT.__constg_swiftt: 0x4d48
   __TEXT.__swift5_builtin: 0x1e0
-  __TEXT.__swift5_reflstr: 0x3b7d
-  __TEXT.__swift5_fieldmd: 0x37a4
-  __TEXT.__swift5_assocty: 0x350
-  __TEXT.__swift5_proto: 0x528
-  __TEXT.__swift5_types: 0x388
-  __TEXT.__oslogstring: 0x249b
-  __TEXT.__swift_as_entry: 0x1f8
-  __TEXT.__swift_as_ret: 0x180
-  __TEXT.__swift_as_cont: 0x334
-  __TEXT.__swift5_capture: 0x1040
-  __TEXT.__swift5_protos: 0xdc
-  __TEXT.__swift5_mpenum: 0x15c
-  __TEXT.__unwind_info: 0x2740
-  __TEXT.__eh_frame: 0x4518
-  __DATA_CONST.__const: 0x8fe8
+  __TEXT.__swift5_reflstr: 0x3e9d
+  __TEXT.__swift5_fieldmd: 0x3a64
+  __TEXT.__swift5_assocty: 0x368
+  __TEXT.__swift5_proto: 0x554
+  __TEXT.__swift5_types: 0x3b0
+  __TEXT.__oslogstring: 0x262b
+  __TEXT.__swift_as_entry: 0x220
+  __TEXT.__swift_as_ret: 0x1a8
+  __TEXT.__swift_as_cont: 0x368
+  __TEXT.__swift5_capture: 0x116c
+  __TEXT.__swift5_protos: 0xe0
+  __TEXT.__swift5_mpenum: 0x164
+  __TEXT.__unwind_info: 0x2900
+  __TEXT.__eh_frame: 0x4a30
+  __DATA_CONST.__const: 0x97d0
   __DATA_CONST.__cfstring: 0x4a0
-  __DATA_CONST.__objc_classlist: 0x2b8
+  __DATA_CONST.__objc_classlist: 0x2c8
   __DATA_CONST.__objc_protolist: 0x2d8
   __DATA_CONST.__objc_imageinfo: 0x8
   __DATA_CONST.__objc_protorefs: 0x168
   __DATA_CONST.__objc_superrefs: 0x58
   __DATA_CONST.__objc_arraydata: 0x18
   __DATA_CONST.__objc_arrayobj: 0x18
-  __DATA_CONST.__auth_got: 0x1118
-  __DATA_CONST.__got: 0x788
-  __DATA_CONST.__auth_ptr: 0xb20
-  __DATA.__objc_const: 0x6338
-  __DATA.__objc_selrefs: 0xb98
+  __DATA_CONST.__auth_got: 0x1128
+  __DATA_CONST.__got: 0x7b8
+  __DATA_CONST.__auth_ptr: 0xb68
+  __DATA.__objc_const: 0x6608
+  __DATA.__objc_selrefs: 0xbc0
   __DATA.__objc_ivar: 0x5c
   __DATA.__objc_data: 0x1028
-  __DATA.__data: 0x7268
-  __DATA.__common: 0x2e8
-  __DATA.__bss: 0x6320
+  __DATA.__data: 0x7530
+  __DATA.__common: 0x2f8
+  __DATA.__bss: 0x6620
   - /System/Library/Frameworks/AVFoundation.framework/Versions/A/AVFoundation
   - /System/Library/Frameworks/AVRouting.framework/Versions/A/AVRouting
   - /System/Library/Frameworks/AppKit.framework/Versions/C/AppKit

   - /System/Library/PrivateFrameworks/Catalyst.framework/Versions/A/Catalyst
   - /System/Library/PrivateFrameworks/ConfigurationProfiles.framework/Versions/A/ConfigurationProfiles
   - /System/Library/PrivateFrameworks/CoreUtils.framework/Versions/A/CoreUtils
+  - /System/Library/PrivateFrameworks/DeviceConfiguration.framework/Versions/A/DeviceConfiguration
   - /System/Library/PrivateFrameworks/LockdownMode.framework/Versions/A/LockdownMode
   - /System/Library/PrivateFrameworks/MediaRemote.framework/Versions/A/MediaRemote
   - /System/Library/PrivateFrameworks/MenuBarClientCore.framework/Versions/A/MenuBarClientCore

   - /usr/lib/swift/libswift_DarwinFoundation1.dylib
   - /usr/lib/swift/libswiftos.dylib
   - /usr/lib/swift/libswiftsimd.dylib
-  Functions: 3302
-  Symbols:   955
-  CStrings:  1369
+  Functions: 3424
+  Symbols:   965
+  CStrings:  1403
 
Symbols:
+ _$s19DeviceConfiguration12UserProviderV6commit6storesySayAA5StoreVG_tYaKFZ
+ _$s19DeviceConfiguration12UserProviderV6commit6storesySayAA5StoreVG_tYaKFZTu
+ _$s19DeviceConfiguration12UserProviderV6delete8storeIDsySayAA15StoreIdentifierCG_tYaKFZ
+ _$s19DeviceConfiguration12UserProviderV6delete8storeIDsySayAA15StoreIdentifierCG_tYaKFZTu
+ _$s19DeviceConfiguration15StoreIdentifierC4name10providerIDACSS_SStcfc
+ _$s19DeviceConfiguration15StoreIdentifierCMa
+ _$s19DeviceConfiguration5StoreV09valuesForB2IDSDySSSDySSs8Sendable_pGGvs
+ _$s19DeviceConfiguration5StoreV7storeIDAcA0C10IdentifierC_tcfC
+ _$s19DeviceConfiguration5StoreV8isActiveSbvs
+ _$s19DeviceConfiguration5StoreVMa
+ _$s19DeviceConfiguration5StoreVMn
+ _$s2os21OSAllocatedUnfairLockVMn
+ _$s7Combine10PublishersO6FilterVMa
+ _$s7Combine10PublishersO6FilterVy_xGAA9PublisherAAMc
+ _$s7Combine9PublisherPAAE6filteryAA10PublishersO6FilterVy_xGSb6OutputQzcF
+ _$s8Catalyst18CATAsyncSerializerC16PreEnqueueActionO14cancelAllTasksyA2EmFWC
+ _$s8Catalyst18CATAsyncSerializerC3run19respectingCancelAll10preEnqueue4name8priority_xSb_AC03PreI6ActionOSgSSSgScPSgxyYaKYAcntYaKs8SendableRzlF
+ _$s8Catalyst18CATAsyncSerializerC3run19respectingCancelAll10preEnqueue4name8priority_xSb_AC03PreI6ActionOSgSSSgScPSgxyYaKYAcntYaKs8SendableRzlFTu
+ _$ss13ManagedBufferCMn
+ _$ss17__CocoaDictionaryV8IteratorC4nextyXl3key_yXl5valuetSgyF
+ _$ss9UnmanagedVMn
+ _NSPOSIXErrorDomain
+ _swift_projectBox
+ _sysctlbyname
- _$sSD5IndexV8_asCocoas02__C10DictionaryVAAVvM
- _$sSD5IndexVMn
- _$ss10_HashTableV14occupiedBucket5afterAB0D0VAF_tF
- _$ss17__CocoaDictionaryV10startIndexAB0D0Vvg
- _$ss17__CocoaDictionaryV5IndexV10dictionaryABvg
- _$ss17__CocoaDictionaryV5IndexV16handleBitPatternSuvg
- _$ss17__CocoaDictionaryV5IndexV3ages5Int32Vvg
- _$ss17__CocoaDictionaryV5IndexV3keyyXlvg
- _$ss17__CocoaDictionaryV5countSivg
- _$ss17__CocoaDictionaryV5index5afterAB5IndexVAF_tF
- _$ss17__CocoaDictionaryV6lookupyyXl3key_yXl5valuetAB5IndexVF
- _$ss17__CocoaDictionaryV9formIndex5after8isUniqueyAB0D0Vz_SbtF
- _kSecCodeInfoUnique
- _objc_retainAutoreleaseReturnValue
CStrings:
+ "AAC Accessibility Intelligence Store"
+ "AAC Visual Intelligence Store"
+ "Alert message when running in a virtual machine. Format specifier is the app name"
+ "Enablement requirement failed: Running in a virtual machine"
+ "Enablement requirement failed: Unable to determine virtual machine status - %{public}s"
+ "Failed to commit device configuration store with error: %{public}s"
+ "Failed to read kern.hv_vmm_present (errno %{public}d)"
+ "Force Quit blocking started"
+ "Force Quit blocking stopped"
+ "Ignoring start() on a torn-down event tap"
+ "Missing participants (unavailable executables): %{public}s"
+ "Running in a virtual machine"
+ "This device is a virtual machine. %@ requires a physical device."
+ "Virtual machine status checked: %{public}s"
+ "_TtC15assessmentagent40AEAConcreteDeviceConfigurationPrimitives"
+ "_TtC15assessmentagentP33_C9A2D2BFC7AE02E87E2ECAF32C861AFA31AEADeviceConfigurationAssertion"
+ "_allowsAccessibilityIntelligence"
+ "_allowsVisualIntelligence"
+ "allowAccessibilityAsk"
+ "allowVirtualMachine"
+ "allowVisualIntelligence"
+ "allowsForceQuit"
+ "allowsForceQuitAntiphony"
+ "com.apple.Accessibility"
+ "com.apple.AutomaticAssessmentConfiguration"
+ "com.apple.assessment.deviceConfiguration.commit"
+ "com.apple.assessment.deviceConfiguration.delete"
+ "com.apple.modelcatalog"
+ "configurationNetworkStateAntiphony"
+ "deviceConfiguration"
+ "executablePath"
+ "forceQuitDialogHotKey"
+ "forceQuitEventTap"
+ "forceQuitFrontmostHotKey"
+ "kern.hv_vmm_present"
+ "keyForValue"
+ "lastCommittedValuesByStore"
+ "selfReference"
+ "shouldDeleteStores"
+ "signingIdentifier"
+ "stateLock"
+ "useDeviceConfigurationProvider"
- "Failed to get security code for %{public}s at %{public}s"
- "Security code primitives unavailable for %{public}s"
- "ae_hexString"
- "allowsNetworkAntiphony"
- "allowsNetworkSubscription"
- "allowsScreenshotsAntiphonySubscription"
- "allowsScreenshotsSubscription"
- "codeDirectoryHash"
```
