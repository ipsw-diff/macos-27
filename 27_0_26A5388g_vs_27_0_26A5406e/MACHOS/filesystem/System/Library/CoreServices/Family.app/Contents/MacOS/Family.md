## Family

> `/System/Library/CoreServices/Family.app/Contents/MacOS/Family`

### Sections with Same Size but Changed Content

- `__TEXT.__cstring`
- `__TEXT.__swift5_assocty`
- `__TEXT.__swift5_builtin`
- `__TEXT.__swift5_proto`
- `__TEXT.__swift5_types`
- `__DATA_CONST.__objc_classlist`
- `__DATA_CONST.__objc_catlist`
- `__DATA_CONST.__auth_ptr`

```diff

-285.0.0.0.0
-  __TEXT.__text: 0x6f04
-  __TEXT.__auth_stubs: 0x720
-  __TEXT.__objc_stubs: 0x5c0
-  __TEXT.__objc_methlist: 0x5e8
-  __TEXT.__const: 0x204
+290.0.0.0.0
+  __TEXT.__text: 0x94f8
+  __TEXT.__auth_stubs: 0x7d0
+  __TEXT.__objc_stubs: 0x6e0
+  __TEXT.__objc_methlist: 0x668
+  __TEXT.__const: 0x234
   __TEXT.__gcc_except_tab: 0x10
   __TEXT.__cstring: 0x190
-  __TEXT.__oslogstring: 0x328
-  __TEXT.__objc_classname: 0x1d5
-  __TEXT.__objc_methname: 0xde6
-  __TEXT.__objc_methtype: 0x5bc
-  __TEXT.__swift5_typeref: 0x348
-  __TEXT.__swift5_capture: 0x68
+  __TEXT.__oslogstring: 0x588
+  __TEXT.__objc_classname: 0x215
+  __TEXT.__objc_methname: 0xec6
+  __TEXT.__objc_methtype: 0x5e5
+  __TEXT.__swift5_typeref: 0x3ac
+  __TEXT.__swift5_capture: 0xd0
   __TEXT.__swift5_reflstr: 0xb6
   __TEXT.__swift5_assocty: 0x18
-  __TEXT.__constg_swiftt: 0x11c
+  __TEXT.__constg_swiftt: 0x134
   __TEXT.__swift5_builtin: 0x14
-  __TEXT.__swift5_fieldmd: 0xd0
+  __TEXT.__swift5_fieldmd: 0xe0
   __TEXT.__swift5_proto: 0xc
   __TEXT.__swift5_types: 0x10
-  __TEXT.__unwind_info: 0x248
-  __TEXT.__eh_frame: 0x40
-  __DATA_CONST.__const: 0x388
+  __TEXT.__swift_as_entry: 0x8
+  __TEXT.__swift_as_ret: 0x8
+  __TEXT.__swift_as_cont: 0x8
+  __TEXT.__unwind_info: 0x2d8
+  __TEXT.__eh_frame: 0x1c0
+  __DATA_CONST.__const: 0x478
   __DATA_CONST.__objc_classlist: 0x20
   __DATA_CONST.__objc_catlist: 0x20
-  __DATA_CONST.__objc_protolist: 0x30
+  __DATA_CONST.__objc_protolist: 0x40
   __DATA_CONST.__objc_imageinfo: 0x8
-  __DATA_CONST.__objc_protorefs: 0x20
-  __DATA_CONST.__auth_got: 0x3a0
-  __DATA_CONST.__got: 0x108
+  __DATA_CONST.__objc_protorefs: 0x28
+  __DATA_CONST.__auth_got: 0x3f8
+  __DATA_CONST.__got: 0x118
   __DATA_CONST.__auth_ptr: 0x98
-  __DATA.__objc_const: 0x9b0
-  __DATA.__objc_selrefs: 0x3e8
-  __DATA.__objc_ivar: 0x10
-  __DATA.__objc_data: 0x238
-  __DATA.__data: 0x248
+  __DATA.__objc_const: 0xa68
+  __DATA.__objc_selrefs: 0x438
+  __DATA.__objc_ivar: 0x14
+  __DATA.__objc_data: 0x250
+  __DATA.__data: 0x2b0
   __DATA.__bss: 0x1a0
   - /System/Library/Frameworks/AppKit.framework/Versions/C/AppKit
   - /System/Library/Frameworks/Cocoa.framework/Versions/A/Cocoa

   - /usr/lib/swift/libswift_Concurrency.dylib
   - /usr/lib/swift/libswiftos.dylib
   - /usr/lib/swift/libswiftsimd.dylib
-  Functions: 175
-  Symbols:   204
-  CStrings:  244
+  Functions: 209
+  Symbols:   217
+  CStrings:  267
 
Symbols:
+ _$s14FamilyCircleUI04makeA12InviteWindow03forE4Code9onDismissSo8NSWindowCSgSS_yyYbctYaF
+ _$s14FamilyCircleUI04makeA12InviteWindow03forE4Code9onDismissSo8NSWindowCSgSS_yyYbctYaFTu
+ _$sScP8rawValues5UInt8Vvg
+ _$sScPMa
+ _$sSo14FAFamilyCircleC06FamilyB0E25getPendingReceivedInvitesSaySo0A6InviteCGyF
+ _$ss12_ArrayBufferV19_getElementSlowPathyyXlSiFyXl_Ts5
+ _$ss18_CocoaArrayWrapperV8endIndexSivg
+ _$sytN
+ _swift_dynamicCastObjCProtocolConditional
+ _swift_task_alloc
+ _swift_task_create
+ _swift_task_dealloc
+ _swift_task_switch
CStrings:
+ "T@\"NSWindow\",&,N,V_inviteWindow"
+ "_TtP6Family45FAUniversalLinkHandlerInvitePresenterProtocol_"
+ "_inviteWindow"
+ "activateIgnoringOtherApps:"
+ "close"
+ "code"
+ "delegate"
+ "dismissInviteWindow"
+ "handleFamilyInvite: invite not in cached payload, presenting directly"
+ "handleFamilyInvite: not signed in, presenting signed-out alert"
+ "handleFamilyInvite: server-tracked invite, routing through System Settings"
+ "handleFamilyInviteWith:completion:"
+ "inviteWindow"
+ "isInviteServerTracked: cache fetch failed, treating as untracked: %s"
+ "makeKeyAndOrderFront:"
+ "presentInviteDirectly: could not build invite window, falling back to System Settings handoff"
+ "presentInviteDirectly: presenter not available, falling back to System Settings handoff"
+ "presentInviteWindow:"
+ "presentInviteWindow: invite window already presented, ignoring"
+ "presentInviteWindow: nil window, terminating"
+ "setInviteWindow:"
+ "v24@0:8@\"NSWindow\"16"
+ "v32@0:8@16@?24"
```
