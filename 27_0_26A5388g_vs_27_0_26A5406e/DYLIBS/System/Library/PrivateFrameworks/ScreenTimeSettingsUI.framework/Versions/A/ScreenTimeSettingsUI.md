## ScreenTimeSettingsUI

> `/System/Library/PrivateFrameworks/ScreenTimeSettingsUI.framework/Versions/A/ScreenTimeSettingsUI`

### Sections with Same Size but Changed Content

- `__TEXT.__swift5_builtin`
- `__TEXT.__swift5_assocty`
- `__TEXT.__swift5_proto`
- `__TEXT.__swift_as_entry`
- `__TEXT.__swift_as_ret`
- `__TEXT.__swift_as_cont`
- `__TEXT.__swift5_protos`
- `__DATA_CONST.__const`
- `__DATA_CONST.__objc_catlist`
- `__DATA_CONST.__objc_protolist`
- `__DATA_CONST.__objc_protorefs`
- `__DATA_CONST.__objc_superrefs`
- `__AUTH_CONST.__objc_intobj`

```diff

-649.0.0.0.0
-  __TEXT.__text: 0x71d38
-  __TEXT.__objc_methlist: 0x26f4
-  __TEXT.__const: 0x33e4
-  __TEXT.__cstring: 0x32a1
-  __TEXT.__gcc_except_tab: 0xb78
-  __TEXT.__oslogstring: 0x18e5
-  __TEXT.__constg_swiftt: 0x1180
-  __TEXT.__swift5_typeref: 0x5278
+655.0.400.0.0
+  __TEXT.__text: 0x72f80
+  __TEXT.__objc_methlist: 0x275c
+  __TEXT.__const: 0x3464
+  __TEXT.__cstring: 0x32e1
+  __TEXT.__gcc_except_tab: 0xbd8
+  __TEXT.__oslogstring: 0x1975
+  __TEXT.__swift5_typeref: 0x5298
+  __TEXT.__swift5_capture: 0x5ec
+  __TEXT.__constg_swiftt: 0x11ac
+  __TEXT.__swift5_fieldmd: 0xa5c
   __TEXT.__swift5_builtin: 0x3c
   __TEXT.__swift5_reflstr: 0x9cd
-  __TEXT.__swift5_fieldmd: 0xa4c
   __TEXT.__swift5_assocty: 0x380
   __TEXT.__swift5_proto: 0xd0
-  __TEXT.__swift5_types: 0xb8
-  __TEXT.__swift5_capture: 0x5dc
+  __TEXT.__swift5_types: 0xbc
   __TEXT.__swift_as_entry: 0x84
   __TEXT.__swift_as_ret: 0x78
   __TEXT.__swift_as_cont: 0x220
   __TEXT.__swift5_protos: 0x8
-  __TEXT.__unwind_info: 0x1c68
-  __TEXT.__eh_frame: 0x1ab8
+  __TEXT.__unwind_info: 0x1cc8
+  __TEXT.__eh_frame: 0x1ac0
   __TEXT.__objc_stubs: 0x0
   __TEXT.__auth_stubs: 0x0
   __TEXT.__objc_classname: 0x0
   __TEXT.__objc_methname: 0x0
   __TEXT.__objc_methtype: 0x0
   __DATA_CONST.__const: 0x170
-  __DATA_CONST.__objc_classlist: 0x120
+  __DATA_CONST.__objc_classlist: 0x128
   __DATA_CONST.__objc_catlist: 0x10
   __DATA_CONST.__objc_protolist: 0xa8
   __DATA_CONST.__objc_imageinfo: 0x8
-  __DATA_CONST.__objc_selrefs: 0x2490
+  __DATA_CONST.__objc_selrefs: 0x24d8
   __DATA_CONST.__objc_protorefs: 0x38
   __DATA_CONST.__objc_superrefs: 0x98
-  __DATA_CONST.__got: 0xb20
-  __AUTH_CONST.__const: 0x24f8
-  __AUTH_CONST.__cfstring: 0x17a0
-  __AUTH_CONST.__objc_const: 0x64b8
+  __DATA_CONST.__got: 0xb68
+  __AUTH_CONST.__const: 0x2548
+  __AUTH_CONST.__cfstring: 0x17c0
+  __AUTH_CONST.__objc_const: 0x6528
   __AUTH_CONST.__objc_intobj: 0x60
-  __AUTH_CONST.__auth_got: 0x1150
-  __AUTH.__objc_data: 0x1168
-  __AUTH.__data: 0x720
+  __AUTH_CONST.__auth_got: 0x11f0
+  __AUTH.__objc_data: 0x1218
+  __AUTH.__data: 0x750
   __DATA.__objc_ivar: 0x1c0
-  __DATA.__data: 0x19e0
+  __DATA.__data: 0x1a20
   __DATA.__bss: 0x1b68
   __DATA.__common: 0x68
   - /System/Library/Frameworks/AVKit.framework/Versions/A/AVKit

   - /System/Library/PrivateFrameworks/ConfigurationEngineModel.framework/Versions/A/ConfigurationEngineModel
   - /System/Library/PrivateFrameworks/FamilyCircle.framework/Versions/A/FamilyCircle
   - /System/Library/PrivateFrameworks/FeatureFlags.framework/Versions/A/FeatureFlags
+  - /System/Library/PrivateFrameworks/GenerativeModels.framework/Versions/A/GenerativeModels
   - /System/Library/PrivateFrameworks/OnBoardingKit.framework/Versions/A/OnBoardingKit
   - /System/Library/PrivateFrameworks/ScreenTimeCore.framework/Versions/A/ScreenTimeCore
   - /System/Library/PrivateFrameworks/ScreenTimeServiceUI.framework/Versions/A/ScreenTimeServiceUI

   - /usr/lib/swift/libswift_Concurrency.dylib
   - /usr/lib/swift/libswiftos.dylib
   - /usr/lib/swift/libswiftsimd.dylib
-  Functions: 2460
-  Symbols:   3157
-  CStrings:  448
+  Functions: 2489
+  Symbols:   3182
+  CStrings:  452
 
Symbols:
+ +[STContentAndPrivacyController keyPathsForValuesAffectingAccessibilityAskAllowed]
+ -[STContentAndPrivacyController accessibilityAskAllowed]
+ -[STContentAndPrivacyController setAccessibilityAskAllowed:]
+ GCC_except_table189
+ GCC_except_table191
+ GCC_except_table284
+ GCC_except_table286
+ GCC_except_table293
+ GCC_except_table301
+ _OBJC_CLASS_$_NSLock
+ _OBJC_CLASS_$_OS_dispatch_queue
+ _OBJC_CLASS_$__TtC20ScreenTimeSettingsUI30STAccessibilityAskAvailability
+ _OBJC_METACLASS_$__TtC20ScreenTimeSettingsUI30STAccessibilityAskAvailability
+ _STIsDeviceChinaSKU
+ __CLASS_METHODS__TtC20ScreenTimeSettingsUI30STAccessibilityAskAvailability
+ __CLASS_PROPERTIES__TtC20ScreenTimeSettingsUI30STAccessibilityAskAvailability
+ __DATA__TtC20ScreenTimeSettingsUI30STAccessibilityAskAvailability
+ __INSTANCE_METHODS__TtC20ScreenTimeSettingsUI30STAccessibilityAskAvailability
+ __METACLASS_DATA__TtC20ScreenTimeSettingsUI30STAccessibilityAskAvailability
+ ___56-[STContentAndPrivacyController accessibilityAskAllowed]_block_invoke
+ ___60-[STContentAndPrivacyController setAccessibilityAskAllowed:]_block_invoke
+ _objc_msgSend$isAccessibilityAskAllowedForUserDSID:error:
+ _objc_msgSend$lock
+ _objc_msgSend$setAccessibilityAskAllowed:forUserDSID:error:
+ _objc_msgSend$unlock
+ _symbolic Say_____G 8Dispatch0A13WorkItemFlagsV
+ _symbolic _____ 20ScreenTimeSettingsUI30STAccessibilityAskAvailabilityC
+ _symbolic _____Sg 10Foundation6LocaleV12LanguageCodeV
+ _symbolic _____XMT 20ScreenTimeSettingsUI30STAccessibilityAskAvailabilityC
- GCC_except_table279
- GCC_except_table281
- GCC_except_table283
- GCC_except_table296
CStrings:
+ "Failed to fetch Accessibility Ask value: %{public}@ . Returning Don't Allow as a fallback."
+ "Failed to save Accessibility Ask value: %{public}@"
+ "accessibility.magnifier"
+ "accessibilityAskAllowed"
```
