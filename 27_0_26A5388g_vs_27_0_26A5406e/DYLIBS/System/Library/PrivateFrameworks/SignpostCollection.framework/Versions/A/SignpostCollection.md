## SignpostCollection

> `/System/Library/PrivateFrameworks/SignpostCollection.framework/Versions/A/SignpostCollection`

### Sections with Same Size but Changed Content

- `__TEXT.__objc_methlist`
- `__TEXT.__const`
- `__TEXT.__gcc_except_tab`
- `__DATA_CONST.__objc_classlist`
- `__DATA_CONST.__objc_catlist`
- `__DATA_CONST.__objc_selrefs`
- `__DATA_CONST.__objc_superrefs`
- `__AUTH_CONST.__const`
- `__AUTH_CONST.__objc_const`
- `__DATA.__data`
- `__DATA_DIRTY.__objc_data`

```diff

-201.0.0.0.0
-  __TEXT.__text: 0x7aec
+203.0.0.0.0
+  __TEXT.__text: 0x7aa8
   __TEXT.__objc_methlist: 0x3fc
   __TEXT.__const: 0x128
   __TEXT.__gcc_except_tab: 0x324
-  __TEXT.__cstring: 0xaa8
+  __TEXT.__cstring: 0xadc
   __TEXT.__oslogstring: 0x355
-  __TEXT.__unwind_info: 0x258
+  __TEXT.__unwind_info: 0x260
   __TEXT.__objc_stubs: 0x0
   __TEXT.__auth_stubs: 0x0
   __TEXT.__objc_classname: 0x0
   __TEXT.__objc_methname: 0x0
   __TEXT.__objc_methtype: 0x0
-  __DATA_CONST.__const: 0x98
+  __DATA_CONST.__const: 0xe0
   __DATA_CONST.__objc_classlist: 0x10
   __DATA_CONST.__objc_catlist: 0x38
   __DATA_CONST.__objc_imageinfo: 0x8

   __DATA_CONST.__objc_superrefs: 0x10
   __DATA_CONST.__got: 0x0
   __AUTH_CONST.__const: 0x4a0
-  __AUTH_CONST.__cfstring: 0x760
+  __AUTH_CONST.__cfstring: 0x7a0
   __AUTH_CONST.__objc_const: 0x588
   __AUTH_CONST.__auth_got: 0x230
   __DATA.__objc_ivar: 0x34

   - /usr/lib/libobjc.A.dylib
   Functions: 165
   Symbols:   618
-  CStrings:  101
+  CStrings:  103
 
Functions:
~ __153-[SignpostSupportObjectExtractor(Notifications) processNotificationsWithIntervalTimeoutInSeconds:shouldCalculateAnimationFramerate:targetQueue:errorOut:]_block_invoke.8 : 276 -> 208
CStrings:
+ "Initialization failure"
+ "Predicate evaluation failure"
```
